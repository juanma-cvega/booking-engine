...
public class CreateSlotUseCaseStepDefinitions extends AbstractUseCaseStepDefinitions {
    ...
    @Then(
            "^a notification of a created slot starting at (.*) and ending at (.*) should be published$")
    public void a_notification_of_a_created_slot_starting_at_and_ending_at_should_be_published(
    ...
    @Then(
            "^a notification of a created slot starting the next day at (.*) and ending at (.*) should be published$")
    public void
            a_notification_of_a_created_slot_starting_the_next_day_at_and_ending_at_should_be_published(
                    String startTime, String endTime) {
    ...
    public void the_slot_should_be_stored_starting_at_and_ending_at(
            String startTime, String endTime) {
        ZonedDateTime startDate = getDateFrom(startTime);
        ZonedDateTime endDate = getDateFrom(endTime);
    ...
    public void the_slot_should_be_stored_starting_the_next_day_at_and_ending_at(
            String startTime, String endTime) {
        ZonedDateTime startDate = getDateFrom(startTime, LocalDate.now(clock).plusDays(1));
        ZonedDateTime endDate = getDateFrom(endTime, LocalDate.now(clock).plusDays(1));
        verifySlotCreatedStored(startDate, endDate);
