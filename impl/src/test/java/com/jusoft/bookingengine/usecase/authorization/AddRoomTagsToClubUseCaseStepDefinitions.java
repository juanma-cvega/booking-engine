...
public class AddRoomTagsToClubUseCaseStepDefinitions extends AbstractUseCaseStepDefinitions {
    ...
    @Autowired private AddRoomTagsToClubUseCase addRoomTagsToClubUseCase;

    @When("^club (.*) is added tag for slot status (.*) to room (.*) in building (.*)$")
    public void club_is_added_tag_for_slot_status_to_room_in_building(
    ...
    public void club_should_have_room_in_building_added_to_its_list_of_buildings(
    ...
    @Then(
            "^room (.*) in building (.*) of club (.*) should have tag of slot status (.*) in its list of tags$")
    public void room_in_building_of_club_should_have_tag_of_slot_status_in_its_list_of_tags(
            Long roomId, Long buildingId, Long clubId, SlotStatus status, DataTable tagsDataTable) {
        List<Tag> tags = tagsDataTable.row(0).stream().map(Tag::of).collect(Collectors.toList());
        Optional<ClubView> club = authorizationManagerComponent.findClubBy(clubId);
        assertThat(club).isPresent();
    ...
    public void club_is_tried_to_be_added_tag_for_slot_status_to_room_in_building(
            Long clubId, SlotStatus status, Long roomId, Long buildingId, DataTable tagsDataTable) {
        List<Tag> tags = tagsDataTable.row(0).stream().map(Tag::of).collect(Collectors.toList());
        storeException(
                () ->
