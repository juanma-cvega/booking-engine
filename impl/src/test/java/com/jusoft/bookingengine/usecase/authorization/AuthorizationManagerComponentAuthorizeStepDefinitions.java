...
public class AuthorizationManagerComponentAuthorizeStepDefinitions
    ...
    @When(
            "^admin verifies credentials of user (.*) to access slot created the (.*) at (.*) in room (.*) in building (.*) in club (.*)$")
    public void
            admin_verifies_credentials_of_user_to_access_slot_created_in_room_in_building_in_club(
                    Long userId,
                    String slotCreationDate,
                    String slotCreationTime,
                    Long roomId,
                    Long buildingId,
                    Long clubId) {
    ...
    @Then(
            "^the user (\\d+) should receive a notification he is not authorized to use room (\\d+) in building (\\d+) in club (\\d+)$")
    public void
            the_user_should_receive_a_notification_he_is_not_authorized_to_use_room_in_building_in_club(
                    Long userId, Long roomId, Long buildingId, Long clubId) {
        assertThat(exceptionThrown).isNotNull();
        assertThat(exceptionThrown).isInstanceOf(UnauthorizedReservationException.class);
