...
public class AddBuildingTagsToMemberUseCaseStepDefinitions extends AbstractUseCaseStepDefinitions {
    ...
    @Autowired private AddBuildingTagsToMemberUseCase addBuildingTagsToMemberUseCase;

    @When("^member (.*) is added tags? to building (\\d+)$")
    public void member_is_added_tags_to_building(
    ...
    public void member_should_have_building_added_to_its_list_of_buildings(
    ...
    public void building_of_member_should_have_tags_in_its_list_of_tags(
            Long buildingId, Long memberId, DataTable tagsDataTable) {
        List<Tag> tags = tagsDataTable.row(0).stream().map(Tag::of).collect(Collectors.toList());
        Optional<MemberView> member = authorizationManagerComponent.findMemberBy(memberId);
        assertThat(member).isPresent();
    ...
    public void member_is_tried_to_be_added_tag_to_building(
            Long memberId, Long buildingId, DataTable tagsDataTable) {
        List<Tag> tags = tagsDataTable.row(0).stream().map(Tag::of).collect(Collectors.toList());
        storeException(
                () ->
