...
public class NotifyOfSlotReservationUseCaseStepDefinitions extends AbstractUseCaseStepDefinitions {
    ...
    @Autowired private NotifyOfSlotReservationUseCase notifyOfSlotReservationUseCase;

    @When(
            "^a notification of a slot reserved by a (.*) with id (\\d+) for slot (\\d+) is published$")
    public void a_notification_of_a_slot_reserved_by_a_with_id_for_slot_is_published(
            UserType userType, Long userId, Long slotId) {
        notifyOfSlotReservationUseCase.notifyOfSlotReservation(
                slotId, SlotUser.of(userId, userType));
    }

    @Then(
            "^a notification of a slot reserved by a user with id (\\d+) for slot (\\d+) should be published$")
    public void a_notification_of_a_slot_reserved_by_a_user_with_id_for_slot_should_be_published(
    ...
    @Then(
            "^a notification of a slot reserved by a class with id (\\d+) for slot (\\d+) should be published$")
    public void a_notification_of_a_slot_reserved_by_a_class_with_id_for_slot_should_be_published(
