...
public class ReplaceSlotsTimetableUseCaseStepDefinitions extends AbstractUseCaseStepDefinitions {
    ...
    @Autowired private ReplaceSlotsTimetableUseCase replaceSlotsTimetableUseCase;
    ...
    public void the_new_slots_are_open(DataTable openDaysDataTable) {
        List<DayOfWeek> openDays =
                openDaysDataTable.row(0).stream()
    ...
    public void the_new_slots_are_open_from_to(String startOpenTime, String endOpenTime) {
        slotsTimetableBuilder.openTimes.add(
                OpenTime.of(startOpenTime, endOpenTime, clock.getZone(), clock));
    }
    ...
    public void the_slot_lifecycle_gets_replaced_its_slots_timetable_with_that_configuration(
            Long roomId) {
        slotLifeCycleManagerComponent.replaceSlotsTimetableWith(
                roomId, slotsTimetableBuilder.build());
    }

    @Then("^the slot lifecycle manager for room (\\d+) should contain the new slots timetable$")
    public void the_slot_lifecycle_manager_for_room_should_contain_the_new_slots_timetable(
            Long roomId) {
        ...
        assertThat(managerView.getSlotsTimetable().getAvailableDays())
                .hasSameElementsAs(slotsTimetable.getAvailableDays());
        assertThat(managerView.getSlotsTimetable().getSlotDurationInMinutes())
                .isEqualTo(slotsTimetable.getSlotDurationInMinutes());
        assertThat(managerView.getSlotsTimetable().getOpenTimesPerDay())
                .hasSameElementsAs(slotsTimetable.getOpenTimesPerDay());
    }

    @When(
            "^the slot lifecycle (\\d+) is tried to be replaced its slots timetable with that configuration$")
    public void
            the_slot_lifecycle_is_tried_to_be_replaced_its_slots_timetable_with_that_configuration(
                    Long roomId) {
        storeException(
                () ->
                        replaceSlotsTimetableUseCase.replaceSlotsTimetable(
                                roomId, slotsTimetableBuilder.build()));
    }

    @Then("^the admin should be notified the new slots timetable for room (.*) cannot be used$")
    public void the_admin_should_be_notified_the_new_slots_timetable_for_room_cannot_be_used(
            Long roomId) {
        SlotsTimetableInvalidException exception =
                verifyAndGetExceptionThrown(SlotsTimetableInvalidException.class);
        assertThat(exception.getRoomId()).isEqualTo(roomId);
        SlotsTimetable slotsTimetable = slotsTimetableBuilder.build();
        assertThat(exception.getSlotsTimetable().getAvailableDays())
                .hasSameElementsAs(slotsTimetable.getAvailableDays());
        assertThat(exception.getSlotsTimetable().getSlotDurationInMinutes())
                .isEqualTo(slotsTimetable.getSlotDurationInMinutes());
        assertThat(exception.getSlotsTimetable().getOpenTimesPerDay())
                .hasSameElementsAs(slotsTimetable.getOpenTimesPerDay());
    }
}
