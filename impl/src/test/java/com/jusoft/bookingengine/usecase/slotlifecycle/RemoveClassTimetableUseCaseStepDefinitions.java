...
public class RemoveClassTimetableUseCaseStepDefinitions extends AbstractUseCaseStepDefinitions {
  ...
  @Then("^a slot lifecycle manager for room (\\d+) should not contain a configuration for class (\\d+)$")
  public void a_slot_lifecycle_manager_for_room_should_not_contain_a_configuration_for_class(Long roomId, Long classId) {
  ...
  @Then("^the admin should receive a notification the class (\\d+) for slot lifecycle manager for room (\\d+) does not exist$")
  public void the_admin_should_receive_a_notification_the_class_for_slot_lifecycle_manager_for_room_does_not_exist(Long classId, Long roomId) {
