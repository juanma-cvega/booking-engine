...
@SpringBootTest(
        classes = {MainConfig.class, MockConfig.class},
        properties = "spring.main.allow-bean-definition-overriding=true")
@DirtiesContext
public class AbstractUseCaseStepDefinitions {

    @Autowired protected MessagePublisher messagePublisher;

    @Autowired protected ClockStub clock;
    ...
    protected ZonedDateTime getNextDateFrom(String time, DayOfWeek dayOfWeek, String zoneId) {
        return ZonedDateTime.of(
                LocalDate.now(clock).plusWeeks(1).with(dayOfWeek), parse(time), ZoneId.of(zoneId));
    }
