Feature: As an admin, I want to remove a pre reservation from a slot lifecycle manager
  ...
  Scenario: As an admin, I want to remove a pre reservation from a slot lifecycle manager
    ...
    Then a slot lifecycle manager 1 should not contain a pre reservation for user 5 next MONDAY at 09:00 from zone UTC
  ...
  Scenario: As an admin, I should not be able to remove a non existing pre reservation
    ...
    Then the admin should receive a notification the pre reservation for next MONDAY at 09:00 from zone UTC for slot lifecycle manager 1 does not exist
