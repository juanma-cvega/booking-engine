...
class BookingManagerComponentImpl implements BookingManagerComponent {
    ...
    public BookingView book(CreateBookingCommand command) {
        ...
        messagePublisher.publish(
                new BookingCreatedEvent(
                        newBooking.getId(), newBooking.getUserId(), newBooking.getSlotId()));
    ...
    public void cancel(long userId, long bookingId) {
        bookingRepository.delete(
                bookingId,
                deleteBookingIfOwner(userId),
                () -> new BookingNotFoundException(bookingId));
        messagePublisher.publish(new BookingCanceledEvent(bookingId));
    }
        ...
        return bookingToDelete -> {
            if (!bookingToDelete.isOwner(userId)) {
                throw new WrongBookingUserException(
                        userId, bookingToDelete.getUserId(), bookingToDelete.getId());
            }
            return true;
        };
    }
    ...
    @Override
    public List<BookingView> findBookingsUntilFor(ZonedDateTime endTime, Set<Long> users) {
