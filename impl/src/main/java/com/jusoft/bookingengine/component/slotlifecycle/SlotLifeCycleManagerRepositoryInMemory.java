...
public class SlotLifeCycleManagerRepositoryInMemory implements SlotLifeCycleManagerRepository {
                ...
                slotLifeCycleManager.getClassesTimeTable().entrySet().stream()
                        .map(
                                entry ->
                                        ClassTimetable.of(
                                                entry.getKey(),
                                                entry.getValue().getReservedSlotsOfDays()))
                        .collect(
                                toMap(
                                        ClassTimetable::getClassId,
                                        classTimetable -> classTimetable)),
                slotLifeCycleManager.getPreReservations().stream()
                        .map(
                                preReservation ->
                                        PreReservation.of(
                                                preReservation.getUserId(),
                                                preReservation.getReservationDate()))
                        .collect(toList()));
    }
    ...
    public void execute(
            long roomId,
            UnaryOperator<SlotLifeCycleManager> modifier,
            Supplier<? extends RuntimeException> notFoundExceptionSupplier) {
        withLock(
                lock,
                () -> {
                    SlotLifeCycleManager slotLifeCycleManagerFound =
                            find(roomId).orElseThrow(notFoundExceptionSupplier);
