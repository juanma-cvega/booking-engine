...
class SlotLifeCycleManagerComponentImpl implements SlotLifeCycleManagerComponent {
    ...
    public SlotLifeCycleManagerView createFrom(long roomId, SlotsTimetable slotsTimetable) {
        SlotLifeCycleManager slotLifeCycleManager =
                new SlotLifeCycleManager(roomId, slotsTimetable);
    ...
    public void replaceSlotsTimetableWith(long roomId, SlotsTimetable slotsTimetable) {
        repository.execute(
                ...
                slotLifeCycleManager ->
                        slotLifeCycleManager.replaceSlotsTimetableWith(slotsTimetable, clock),
    ...
    public void modifyAuctionConfigFor(long roomId, AuctionConfigInfo auctionConfigInfo) {
        repository.execute(
                roomId,
                slotLifeCycleManager ->
                        slotLifeCycleManager.replaceAuctionConfigWith(auctionConfigInfo),
    ...
    public void addPreReservation(long roomId, PreReservation preReservation) {
        repository.execute(
                roomId,
                slotLifeCycleManager ->
                        slotLifeCycleManager.addPreReservation(preReservation, clock),
                supplySlotLifeCycleManagerNotFoundException(roomId));
    }
    ...
    public void findNextSlotStateFor(long slotId, long roomId, ZonedDateTime slotStartTime) {
        SlotLifeCycleManager slotLifeCycleManager =
                repository
                        .find(roomId)
                        .orElseThrow(supplySlotLifeCycleManagerNotFoundException(roomId));
        NextSlotState nextSlotState =
                slotLifeCycleManager.nextStateFor(slotId, slotStartTime, clock);
    ...
    private SlotLifeCycleManager findOrFail(long roomId) {
        return repository
                .find(roomId)
                .orElseThrow(() -> new SlotLifeCycleManagerNotFoundException(roomId));
    }

    private List<SlotLifeCycleManagerView> createFrom(
            List<SlotLifeCycleManager> slotLifeCycleManagers) {
