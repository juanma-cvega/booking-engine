...
class SlotLifeCycleManager {
    ...
    @NonNull private final SlotsTimetable slotsTimetable;

    @NonNull private final AuctionConfigInfo auctionConfigInfo;

    @NonNull private final Map<Long, ClassTimetable> classesTimetable;

    @NonNull private final List<PreReservation> preReservations;
    ...
    SlotLifeCycleManager(
            long roomId,
            SlotsTimetable slotsTimetable,
            AuctionConfigInfo auctionConfigInfo,
            Map<Long, ClassTimetable> classesTimetable,
            List<PreReservation> preReservations) {
        this.roomId = roomId;
    ...
    NextSlotState nextStateFor(long slotId, ZonedDateTime slotStartTime, Clock clock) {
        // The order of suppliers defines the preferences in terms of finding the next state for a
    ...
    private Supplier<Optional<NextSlotState>> isReservedForClassState(
            long slotId, ZonedDateTime startTime, Clock clock) {
        return () ->
                classesTimetable.values().stream()
    ...
    private Supplier<Optional<NextSlotState>> isPreReservedState(
            long slotId, ZonedDateTime slotStartTime) {
        return () ->
                preReservations.stream()
                        .filter(
                                preReservation ->
                                        preReservation.getReservationDate().isEqual(slotStartTime))
                        .findFirst()
                        .map(
                                preReservation ->
                                        new PreReservedState(
    ...
    SlotLifeCycleManager replaceAuctionConfigWith(AuctionConfigInfo auctionConfigInfo) {
        return new SlotLifeCycleManager(
                roomId, slotsTimetable, auctionConfigInfo, classesTimetable, preReservations);
    }
    ...
    SlotLifeCycleManager addClass(ClassTimetable classTimetable, Clock clock) {
        ...
        if (isPreReservationWithinClassTimetable(classTimetable, clock)
                || isClassTimetableOverlappingAnyClass(classTimetable)) {
    ...
    private boolean isClassTimetableOverlappingAnyClass(ClassTimetable classTimetable) {
        return classesTimetable.values().stream()
                .anyMatch(
                        configuredClassTimetable ->
                                configuredClassTimetable.intersectsWith(
                                        classTimetable.getReservedSlotsOfDays()));
    }

    private boolean isPreReservationWithinClassTimetable(
            ClassTimetable classTimetable, Clock clock) {
    ...
    SlotLifeCycleManager addPreReservation(PreReservation preReservation, Clock clock) {
        preReservations.removeAll(
                findPassedPreReservations(
                        clock)); // Cleans up the list to avoid iterating through stale data
    ...
    private boolean isReservationDateAlreadyTaken(ZonedDateTime reservationDate, Clock clock) {
        return isSlotTimeAlreadyPreReserved(reservationDate)
                || anyClassContains(reservationDate, clock);
    }
    ...
    SlotLifeCycleManager removePreReservation(ZonedDateTime slotStartTime) {
        PreReservation preReservationFound =
                preReservations.stream()
                        .filter(
                                preReservation ->
                                        preReservation.getReservationDate().isEqual(slotStartTime))
                        .findFirst()
                        .orElseThrow(
                                () -> new PreReservationNotFoundException(roomId, slotStartTime));
    ...
    private boolean anyClassContains(ZonedDateTime reservationDate, Clock clock) {
        return classesTimetable.values().stream()
                .anyMatch(
                        classTimetable ->
                                classTimetable.getReservedSlotsOfDays().stream()
                                        .anyMatch(
                                                reservedSlotsOfDay ->
                                                        reservedSlotsOfDay.contains(
                                                                reservationDate, clock)));
    }

    SlotLifeCycleManager replaceSlotsTimetableWith(SlotsTimetable slotsTimetable, Clock clock) {
        if (!areCurrentPreReservationsCoveredBy(slotsTimetable, clock)
                || !areCurrentClassesCoveredBy(slotsTimetable)) {
            SlotsTimetable slotsTimetableDetails =
                    SlotsTimetable.of(
                            slotsTimetable.getSlotDurationInMinutes(),
                            slotsTimetable.getOpenTimesPerDay(),
                            slotsTimetable.getAvailableDays());
    ...
    private boolean areCurrentPreReservationsCoveredBy(SlotsTimetable slotsTimetable, Clock clock) {
        return preReservations.stream()
                .map(PreReservation::getReservationDate)
                .allMatch(reservationDate -> slotsTimetable.isCovered(reservationDate, clock));
    }
