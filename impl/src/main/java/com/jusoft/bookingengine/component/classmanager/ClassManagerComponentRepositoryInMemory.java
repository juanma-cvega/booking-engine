...
class ClassManagerComponentRepositoryInMemory implements ClassManagerComponentRepository {
    ...
    public void save(Class newClass) {
        if (store.get(newClass.getId()) != null) {
            throw new IllegalArgumentException(
                    "Unable to save new entity. Entity already in store");
        }
        ...
        try {
            Class classRemoved =
                    store.computeIfPresent(
                            classId,
                            (id, classFound) -> remapToNullIfOrReturn(condition, classFound));
    ...
    public void execute(long classId, UnaryOperator<Class> modifier) {
        Class classModified =
                store.computeIfPresent(classId, (id, classFound) -> modifier.apply(classFound));
    ...
    public List<Class> findByBuildingId(long buildingId) {
        return store.values().stream()
                .filter(classFound -> classFound.getBuildingId() == buildingId)
                .collect(toList());
    }
}
