...
class SchedulerComponentInMemory implements SchedulerComponent {
    ...
    public void schedule(ZonedDateTime executionTime, Message scheduledEvent) {
        ScheduledFuture<?> schedule =
                scheduledExecutorService.schedule(
                        createCommand(scheduledEvent), getDelay(executionTime), MILLISECONDS);
    ...
    private Runnable createCommand(Message scheduledEvent) {
        // Executes the publishing in a different thread to avoid contention if the publisher
        // implementation calls the
        // listener in the same thread
