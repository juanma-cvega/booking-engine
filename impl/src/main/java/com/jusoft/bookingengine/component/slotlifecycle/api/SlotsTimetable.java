...
public class SlotsTimetable {
    ...
    public static SlotsTimetable of(
    ...
    public boolean isCovered(ReservedSlotsOfDay reservedSlotsOfDays) {
        return availableDays.contains(reservedSlotsOfDays.getDayOfWeek())
                && reservedSlotsOfDays.getSlotsStartTime().stream().allMatch(this::isTimeValid);
    }

    /**
     * Returns whether a given {@link ZonedDateTime} is covered
     *
     * @param date The date to verify. IMPORTANT: it's transformed to use {@link ZoneId} UTC before
     *     start validation.
     ...
     * @param localTime {@link LocalTime} to verify. IMPORTANT: it's considered to be a {@link
     *     LocalTime} related to UTC
     * @return whether the given parameters are covered
     */
    ...
    private boolean isStartTimeValidFor(SystemLocalTime startTime, OpenTime openTime) {
        return isStartTimeWithinOpenTime(startTime, openTime)
                && isStartTimeAtStartOfSlot(startTime, openTime);
    }

    private boolean isStartTimeWithinOpenTime(SystemLocalTime startTime, OpenTime openTime) {
        return startTime.compareTo(openTime.getStartTime()) >= 0
                && startTime.compareTo(openTime.getEndTime()) <= 0;
    }

    private boolean isStartTimeAtStartOfSlot(SystemLocalTime reservationTime, OpenTime openTime) {
        long timeUntilEndOpenTime =
                reservationTime.until(openTime.getEndTime(), ChronoUnit.MINUTES);
