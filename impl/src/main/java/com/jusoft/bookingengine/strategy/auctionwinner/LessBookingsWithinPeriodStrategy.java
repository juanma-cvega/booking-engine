...
class LessBookingsWithinPeriodStrategy implements AuctionWinnerStrategy {
    ...
    public Optional<Long> findWinner(Set<Bid> bids) {
        ...
        if (!bids.isEmpty()) {
            ZonedDateTime endPeriod =
                    ZonedDateTime.now(clock).plusDays(config.getEndRangeTimeInDays());
            Set<Long> bidders = bids.stream().map(Bid::getUserId).collect(toSet());
            List<BookingView> usersBookingsFound =
                    bookingManagerComponent.findBookingsUntilFor(endPeriod, bidders);
            Set<Long> usersWithLessBookings =
                    bidders.stream()
                            .collect(groupingByUserToNumberOfBookings(usersBookingsFound))
                            .entrySet()
                            .stream()
                            .collect(groupingByNumberOfBookingsToUsers())
                            .entrySet()
                            .stream()
    ...
    private long bookingsFor(List<BookingView> bookings, Long user) {
        return bookings.stream().filter(booking -> booking.userId() == user).count();
    }

    private Collector<Long, ?, Map<Long, Long>> groupingByUserToNumberOfBookings(
            List<BookingView> usersBookingsFound) {
        return toMap(user -> user, user -> bookingsFor(usersBookingsFound, user));
    }

    private Collector<Map.Entry<Long, Long>, ?, Map<Long, Set<Long>>>
            groupingByNumberOfBookingsToUsers() {
